...
function updatePlot(plot_data,data){
   ...
   for (var j = 0; j < n; j++){
      ...
      //label y-axis
      vis.append("text")
         .attr("transform","translate(" + (margin.left + (j/(n-1)) * width) + "," + (margin.top - height/30) +")")
         .style("text-anchor","middle")
         .text(data[j]);
   }
   //Draw the lines 
   ...
   //add the polyline
   var line = d3.line()
      //.defined(function (d) {return !isNaN(d.find(x_label)) && !isNaN(d.find(y_label))}) //remove undefined points
      .x(function(d) {return xScale(d)})
      .y(function (d) {return yScale(d.find(y_labels[int(xScale(d))]))})
