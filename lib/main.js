...
function updatePlot(plot_data){
   ...
   //Scales and Axes
   var xScale = d3.scaleLinear().domain([0, d3.max(plot_data, function(d){return d.find(x_label)})]).range([0, width]);
   var yScale  = d3.scaleLinear().domain([0, d3.max(plot_data, function(d){return d.find(y_label)})]).range([height,0]);
   ...
   var line = d3.line()
      ...
      .x(function (d) {return xScale(d.find(x_label))})
      .y(function (d) {return yScale(d.find(y_label))})
