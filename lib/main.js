...
function main() {
   $(document).ready(function(){
      ...
      //test data
      var data = [{x:1,y:4},{x:5,y:2},{x:19,y:11},{x:30,y:21} ];
      var margin = {top: 10, right: 20, bottom: 50, left: 50};
      var w = 500, h = 500;

      //Scales and Axes
      var xScale = d3.scaleLinear().domain([0, d3.max(data, function(d){return d.x})]).range([0, w]);
      var yScale  = d3.scaleLinear().domain([0, d3.max(data, function(d){return d.y})]).range([h,0]);
