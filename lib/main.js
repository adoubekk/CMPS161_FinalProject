...
function main() {
   $(document).ready(function(){
      ...
      //test data
      var test_data = [new Map(x:1,y:4},{x:5,y:2},{x:19,y:11},{x:30,y:21} ];
      updatePlot(test_data);
...
function updatePlot(plot_data){
   var margin = {top: 10, right: 20, bottom: 50, left: 50}; //conventional margins
   var width = 500 - margin.left - margin.right, height = 500 - margin.left - margin.right;

   //Scales and Axes
   var xScale = d3.scaleLinear().domain([0, d3.max(plot_data, function(d){return d.x})]).range([0, width]);
   var yScale  = d3.scaleLinear().domain([0, d3.max(plot_data, function(d){return d.y})]).range([height,0]);
   var xAxis = d3.axisBottom(xScale);
   var yAxis = d3.axisLeft(yScale);
   
   //create the SVG DOM element and assign its attributes
   var vis = d3.select("#plot").append("svg:svg")
      .attr("width",width + margin.left + margin.right)
      .attr("height",height + margin.top + margin.bottom)
      .append("g") //create the group "g" and shift it over by the margin values
