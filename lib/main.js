...
function main() {
   $(document).ready(function(){
      ...
      $("#SetSelectors").change(function(){
         ...
         updateLegend(legend_tags);
         legendLen = legend_tags.length;
         updateColors(records,data,sets,tags,brushedDefault);
      });
      
      $("#LegendSelectors").change(function(){
         updateColors(records,data,sets,tags,brushedDefault);
         $(".brush").trigger("change");
      });
      $
...
function updatePlot(plot_data,data,sets,tags,title){
               ...
               for (var j = 0; j < brushes.length; j++){
                  ...
                  if (extent){
                     for (var k = 0; k < plot_data.length; k++){
                        data_point = plot_data[k].getPairs(show.data)[j][1];
                        if( yScales[j](data_point) <= extent[0]   || yScales[j](data_point) >= extent[1]){
                           brushed[k] = false;
                        }
                     }
                  }
   ...
   var colorScale = d3.scaleOrdinal().domain(show.subsets).range(d3.schemeCategory20);
