...
function updatePlot(plot_data,data){
   ...
   for (var i = 0; i < data.length; i++){
      yScales.push(d3.scaleLinear().domain([0, d3.max(plot_data, function(d){return d.find(data[i])})]).range([height,0]));
      yAxes.push(d3.axisLeft(yScales[i]));
   }
   var n = yAxes.length;
   for (var i = 0; i < n; i++){
      range_Points.push( (i/(n-1)) * width);
   } 
   var xScale = d3.scaleOrdinal().domain(data).range(range_Points);
   ...
   //add the polyline
   var line = d3.line()
      .defined(function (d) {return !isNaN(d[1])}) //remove undefined points (this causes breaks in lines, need to find better soln.)
      .x(function(d,i) {//console.log(range_Points[i%n] + data[i%n] + d.find(data[i%n]) ); 
                        return range_Points[i%n]; } )
      .y(function (d,i) { return yScales[i%n]( d[1] ) })
     
   for (var i = 0; i < 10; i++){
      console.log(Array.from(plot_data[i].vals.entries()));
      vis.append("path")
         .datum(Array.from(plot_data[i].vals.entries()) )
